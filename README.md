...
# Как выглядит процесс разработки до внедрения библиотеки MagicInputs
...
# Дублирование кода без внедрения библиотеки MagicInputs
...
# Лаконичное решение с помощью библиотеки MagicInputs
...
```
class SimpleCommand extemds MagicInputCommand {

    /**
    * @MagicInput\HttpGet('name')
    * @MagicInput\CliArg(1)
    * @MagicInput\InputArg('name')
    * @MagicInput\Array(0)
    * @MagicInput\Json('$.data.name')
    **/
    public $name;

    /**
    * @MagicInput\HttpPost('description')
    * @MagicInput\HttpPost('descr')
    * @MagicInput\CliArg(2)
    * @MagicInput\InputOption('description')
    * @MagicInput\Array(1)
    * @MagicInput\Json('$.data.options.description')
    **/
    public $description;
}
```
И в зависимости от контекста запуска MagicInputCommand будет вытаскивать нужное значение из нужного контекста. Таким образом, привязка команды к разным контекстам запуска будет находиться в одной точке, а не будет разбросана по всему коду.

# Ещё больше магических режимов!

Библиотека MagicInputs не называлась бы так, если бы не обладала особой магией! В ней присутствует режим при котором разработчику не придётся писать аннотации для свойств класса. Но этот способ менее надёжен и вы используете его на свой страх и риск. Рекомендуется 100% покрытие тестами кода ваших приложений которые используют этот режим!

Суть данного режима в том, что если именя полей класса 1 в 1 соответствуют названиям полей во входных данных, то нет смысла описывать эти правила явно с помощью аннотаций. Внутренний механизм MagicInputs определит это самостоятельно и извлечёт нужные свойства из нужного контекста.

Если же формат входных данных не подразумевает указания имён полей, то MagicInputs также сможет сделать маппинг автоматически. В этом случае порядок объявления свойств класса должен соответствовать порядку следования элементов во входных данных.

Также в будущем может быть добавлена поддержка типов полей для последующей валидации данных и преобразованию их к нужному типу. Также можно реализовать режим при котором будет учитываться не только порядок следования, но и их типы для более магического и экспериментального заполнения данных.

Управление режимами может выполняться при помощи аннотаций к класу сущности.

```
/**
    * @MagicInput\EnableMagic(true)
    * @MagicInput\EnableValidator(true)
    * @MagicInput\EnableForceCast(false)
    * @MagicInput\EnableSequenceOrder(false)
    **/
class SimpleCommand extemds MagicInputCommand {

    /**
    * @MagicInput\StringType()
    * @MagicInput\NotNullType()
    **/
    public $name;
}
```
